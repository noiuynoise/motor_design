...
def GetResultNames(entry: Dict[str, any]) -> List[str]:
    result_names = []
    for key in entry.keys():
        if isinstance(entry[key], dict):
            for key2 in GetResultNames(entry[key]):
                result_names.append(key + '_' + key2)
        else:
            result_names.append(key)
    return result_names

def CombineRuns(run_location: str):
    run_folders = os.listdir(args.run_location)
    num_folders = sum(
        [1 if os.path.isdir(args.run_location + '/' + x) else 0 for x in run_folders])
    print(f'ingesting {num_folders} folders\n')

    # load simulation config
    config_file = args.run_location + '/simulation_config.json'
    ...
    sim_angle_step = sim_config['angle_step']
    data_shape = []
    ...
    for run_folder in run_folders:
        ...
        if file_to_ingest is None:
            print(
                f'error: {run_location} does not contain results.json. Skipping\n')
            continue
        ...
        if combined_data is None:
            ...
            for key in GetResultNames(data[0]):
                combined_data[key] = np.empty(
                    (0, num_angle_steps), np.double)
            continue

        if num_angle_steps != len(data):
            print(
                f'error: {run_location} has {len(data)} angle steps, expected {num_angle_steps}. Skipping\n')
            continue

        for key in data[0].keys():
            if isinstance(data[0][key], dict):
                for key2 in data[0][key].keys():
                    combined_data[key + '_' + key2] = np.append(combined_data[key + '_' + key2], np.array(
                        [[data[i][key][key2] for i in range(num_angle_steps)]]), axis=0)
            else:
                combined_data[key] = np.append(combined_data[key], np.array(
                    [[data[i][key] for i in range(num_angle_steps)]]), axis=0)

        ingested_folders = ingested_folders + 1

    print(f'ingested {ingested_folders} runs\n')
    ...
    for key in combined_data.keys():
        try:
            ...
            pass
        np.save(args.run_location + '/combined/' +
                key + '.npy', combined_data[key])
    ...
    for key in combined_data.keys():
        ...
        ordered_data[key] = np.empty(
            (int(a_max / a_step) + 1, int(b_max / b_step) + 1, int(angle_max / angle_step) + 1), np.double)
    ...
    for i in range(len(a_current)):
        for key in combined_data.keys():
            ...
            ordered_data[key][a_current_index, b_current_index,
                              angle_index] = flattened_data[key][i]
    ...
    for key in ordered_data.keys():
        try:
            ...
            pass
        np.save(args.run_location + '/combined/' +
                key + '_ordered.npy', ordered_data[key])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='data ingest')
    parser.add_argument('run_location', type=str,
                        help='location of run folder')
    args = parser.parse_args()

    CombineRuns(args.run_location)
